...
import { saveDeclarationAction } from '@/lib/actions/maintenance-actions';
...
export function DeclarationForm({ operation }: { operation: Operation }) {
  ...
  function onSubmit(values: DeclarationFormValues) {
    startTransition(async () => {
      const result = await saveDeclarationAction(operation.id, values);
      if (result.success && result.declarationId) {
        toast({
          title: 'Succès',
          description: 'La déclaration de panne a été enregistrée.',
        });
            ...
            <div className="lg:col-span-2 space-y-8">
                <Card>
                    ...
                    <CardContent className="grid md:grid-cols-2 gap-6">
                        <p><strong>Date de panne:</strong> {operation.date_entree}</p>
                        <p><strong>Type de matériel:</strong> {operation.marque} {operation.designation}</p>
                        <p><strong>Immatriculation:</strong> <span className='font-mono'>{operation.matricule}</span></p>
                        <p><strong>Affectation:</strong> {operation.affectation || 'N/A'}</p>
                        <div className="md:col-span-2">
                           <p><strong>Déclaration de l'utilisateur:</strong></p>
                           <p className="text-muted-foreground p-2 bg-muted rounded-md">{operation.panne_declaree}</p>
                        </div>
